...
class Hmmer:
    ...
    def check_euk_contamination(self, output_path, euk_free_output_path, input_path, run_stats, input_file_format, check_total_euks, threads, evalue, raw_reads, base, cmd_log):
        ...
        if check_total_euks:
            nhmmer_cmd = "nhmmer --cpu %s %s --tblout %s /srv/db/graftm/0/HMM/Euk.hmm" % (threads, evalue, output_path)
        ...
        for line in [x.rstrip().split() for x in open(output_path, 'r') if not x.startswith('#')]:
            ...
            try:
                if float(run_stats['reads'][read_name]['eval']) >= float(eval):
                    if ali_length < 0:
