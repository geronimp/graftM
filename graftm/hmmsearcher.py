...
class HmmSearcher:
    ...
    def hmmsearch(self, input_pipe, hmms, output_files, write_orfs):
        ...
        while len(queue) > 0:
            ...
            cmd = self._hmm_command(input_pipe, pairs_to_run, write_orfs)
    ...
    def _hmm_command(self, input_pipe, pairs_to_run, write_orfs):
        ...
        if write_orfs:
            with tempfile.NamedTemporaryFile(prefix='written_orfs_') as \
                                                            written_orfs_file:
                hmmsearch_cmd = "%s > %s ; %s ; rm %s" % (input_pipe, 
                                                          written_orfs_file.name,
                                                          self._individual_concat_hmm_command(element[0][0],
                                                                                              element[0][1],
                                                                                              element[1],
                                                                                              written_orfs_file.name),
                                                          written_orfs_file.name)
    ...
    def _individual_concat_hmm_command(self, hmm, output_file, num_cpus, sequences):
        return "hmmsearch %s --cpu %s -o /dev/null --noali --domtblout %s %s %s" % (self._extra_args,
                                                                         num_cpus,
                                                                         output_file,
                                                                         hmm,
                                                                         sequences)
