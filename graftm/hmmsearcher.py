...
class HmmSearcher:
    ...
    def hmmsearch(self, input_pipe, hmms, output_files, concat_hmm):
        ...
        while len(queue) > 0:
            ...
            cmd = self._hmm_command(input_pipe, pairs_to_run, concat_hmm)
    ...
    def _hmm_command(self, input_pipe, pairs_to_run, write_orfs):
        ...
        if write_orfs:
            with tempfile.NamedTemporaryFile(prefix='written_orfs_') as \
                                                            written_orfs_file:
                hmmsearch_cmd = self._individual_concat_hmm_command(element[0][0],
                                                                    element[0][1],
                                                                    element[1],
                                                                    input_pipe)
